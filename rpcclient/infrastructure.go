...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	case nil:
		log.Debugf("Detected brond version: %v", info.Version)
		version := Btcd
	...
	case *btcjson.RPCError:
		if err.Code != btcjson.ErrRPCMethodNotFound.Code {
			return 0, fmt.Errorf("unable to detect brond version: "+
				"%v", err)
		}

	default:
		return 0, fmt.Errorf("unable to detect brond version: %v", err)
	}
